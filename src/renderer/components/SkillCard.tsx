...
const SkillCard: Component<SkillCardProps> = (props) => {
  ...
  return (
    <div
      ...
      onClick={() => props.onToggle(props.skill.name)}
    >
      ...
      <Show when={props.viewMode === "detailed"}>
        <Show when={props.skill.directory !== "root"}>
          <div class="text-xs text-[#86868b] mt-1">{props.skill.directory}/</div>
        </Show>
        <Show when={props.skill.description}>
          <div class="text-xs text-[#6b7280] mt-1">{props.skill.description}</div>
        </Show>
      </Show>
    </div>
  )
